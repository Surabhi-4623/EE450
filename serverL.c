...
#define FILENAME "./literature.txt"
...
int main() {
 ...
 // Set up server address for ServerL(referred Beej)
    struct sockaddr_in serverLAddress;
    serverLAddress.sin_family = AF_INET;
    serverLAddress.sin_addr.s_addr = INADDR_ANY;
    serverLAddress.sin_port = htons(ServerL_PORT);

    // Bind the socket to the specified address and port for ServerL(referred Beej)
    if (bind(udpSocket, (struct sockaddr *)&serverLAddress, sizeof(serverLAddress)) == -1) {
        perror("Error binding socket");
        close(udpSocket);
        exit(EXIT_FAILURE);
    }
    
    printf("ServerL is up and running using UDP on port %d\n", ServerL_PORT);
   ...
   // printf("Book count:%d\n",bookCount);
    ...
    printf("ServerL finished sending the availability status of code %s to the main server using UDP on port %d",truncatedCode,ServerL_PORT);
    break;
      }
    }
   ...
   if(!f){
    ...
    //Send messae to ServerM telling that book doesn;t exist
    char message[1024];
    sprintf(message, "The requested book %s doesn't exist\n", truncatedCode);
    sendto(udpSocket, message, strlen(message), 0, (struct sockaddr*)&serverMAddress, serverMAddressLength);
    }  
   }
  }
