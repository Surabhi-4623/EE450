...
int main() {
 ...
 // Set up server address for ServerH(referred Beej)
    struct sockaddr_in serverHAddress;
    serverHAddress.sin_family = AF_INET;
    serverHAddress.sin_addr.s_addr = INADDR_ANY;
    serverHAddress.sin_port = htons(ServerH_PORT);

    // Bind the socket to the specified address and port for ServerH(referred Beej)
    if (bind(udpSocket, (struct sockaddr *)&serverHAddress, sizeof(serverHAddress)) == -1) {
        perror("Error binding socket");
        close(udpSocket);
        exit(EXIT_FAILURE);
    }
    
    printf("ServerH is up and running using UDP on port %d\n", ServerH_PORT);
   ...
   // printf("Book count:%d\n",bookCount);
    ...
    printf("ServerH finished sending the availability status of code %s to the main server using UDP on port %d",truncatedCode,ServerH_PORT);
    break;
      }
    }
